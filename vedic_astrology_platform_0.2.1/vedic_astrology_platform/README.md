...
**Version 0.2.1 — Varga Registry**

一個以透明、正確、可驗證及遵循古典 Jyotisha 為核心的開源吠陀占星平台。此版本在相容優先的模組化核心上加入統一的 Varga Registry，修正 D4／D7／D12 中文說明缺漏，並讓分盤名稱、說明、順序與參考資料由單一註冊表管理。
...
- Fixed the Traditional Chinese `KeyError` affecting D4, D7, and D12 descriptions.
- Added `core/varga_registry.py` as the single source of truth for supported varga order, codes, bilingual labels, descriptions, and references.
- The Streamlit divisional-chart selector and chart-generation loop now read from the registry.
- Added registry completeness tests to prevent missing localized UI text in future releases.
